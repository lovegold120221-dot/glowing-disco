...
function ControlTray({ children }: ControlTrayProps) {
  ...
  const isSpeaking = useVAD(audioRecorder, 0.05, 3000); // Increased to 3s to detect end of speech
  const connectButtonRef = useRef<HTMLButtonElement>(null);
  const { session, user } = useAuth();
  const { setUserSpeaking } = useSettings();
  ...
  // Sync VAD state with settings store
  useEffect(() => {
    setUserSpeaking(isSpeaking);
  }, [isSpeaking, setUserSpeaking]);
  ...
  useEffect(() => {
    ...
    root.style.setProperty('--mic-volume', micVolume.toString());
    return () => {
      root.style.removeProperty('--mic-volume');
    };
  }, [micVolume]);
